...
# Load configuration
CONFIG_FILE = "app/config.json"  # Ensure this file exists in the same directory
...
def generate_normal_log(roles, endpoints, network_subnet, extended_days):
    ...
    weights = list(ROLE_WEIGHTS_NORMAL.values())
    role = random.choices(role_choices, weights=weights, k=1)[0]
...
def save_logs_to_csv(logs: list[dict], master_file: str, inference_file: str, mode: str = 'w'):
    """
    ...
    with open(master_file, mode, newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames_master)
        if mode == 'w' or not master_exists:
            writer.writeheader()
    ...
    with open(inference_file, mode, newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames_inference)
        if mode == 'w' or not inference_exists:
            writer.writeheader()
        for log in logs:
...
def main():
    ...
    # Roles and user IDs
    # Since "ROLES" are defined in config.json, use them directly
    roles = config["ROLES"]
    ...
    save_logs_to_csv(logs, master_file, inference_file, mode='w')  # 'w' to overwrite
