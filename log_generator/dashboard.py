...
# # Define unusual hour range:
# UNUSUAL_HOUR_START = 22  # 10 PM
# UNUSUAL_HOUR_END = 6     # 6 AM (so hours <6 or >=22 are unusual)
...
#         # If you want extra columns, add them here
#     ]
...
#     if df_arch.empty:
#         return df_arch  # no data at all
#     if df_infer.empty:
#         return df_arch   # we do have archive logs but no inference results

#     df_merged = df_arch.merge(df_infer, on="LogID", how="left")

#     # If the same column name appears in both DFS, Pandas creates col_x, col_y
#     # We unify them so e.g. 'Role' doesn't remain 'Role_x' and 'Role_y'.
#     # We'll do it for Role, Endpoint, Timestamp. Add more if needed.
...
#             # Merge them: we prioritize xcol if not null, else ycol
...
#     # Also unify predicted_anomaly if it got splitted (rarely)
...
#     # If predicted_anomaly doesn't exist or is partial, ensure the column is numeric
#     if "predicted_anomaly" not in df_merged.columns:
#         df_merged["predicted_anomaly"] = float('nan')
#     df_merged["predicted_anomaly"] = df_merged["predicted_anomaly"].fillna(0).astype(int)

#     if "anomaly_score" not in df_merged.columns:
...
# def main():
#     st.set_page_config(
#         page_title="Enhanced Real-Time Anomaly Dashboard (Unified Columns)",
#         layout="wide"
#     )
...
#     st.title("Real-Time Anomaly Detection Dashboard with Proper Column Unification")
...
#     # Let's see if we actually have 'Role', 'Endpoint', 'Timestamp' now
#     # We'll do the same approach as before
...
#     col2.metric("Anomalies", f"{int(anomaly_count)}")
#     col3.metric("Anomaly Ratio", f"{anomaly_ratio:.2%}")

#     # Alert
#     if anomaly_ratio >= ANOMALY_RATIO_THRESHOLD or anomaly_count >= ANOMALY_COUNT_THRESHOLD:
...
#     else:
#         st.write("No 'Role' column found even after unification. Please confirm pipeline includes Role data.")

#     # B) Endpoint-based
#     st.subheader("B) Endpoint-based Analysis")
#     if "Endpoint" in df_merged.columns:
#         ep_usage = df_merged["Endpoint"].value_counts().reset_index()
#         ep_usage.columns = ["Endpoint","count_usage"]
#         ep_usage = ep_usage.head(10)
#         chart_ep_usage = (
#             alt.Chart(ep_usage)
#             .mark_bar(size=25)
#             .encode(
...
#     else:
#         st.write("No 'Endpoint' column found even after unification. Confirm pipeline includes Endpoint data.")

#     # C) IP-based
#     st.subheader("C) IP-based Analysis")
...
#     # D) Time-based
#     st.subheader("D) Time-based Analysis (Daily)")
#     if "Timestamp" in df_merged.columns and df_merged["Timestamp"].notna().sum() > 0:
#         df_merged["date"] = df_merged["Timestamp"].dt.date

#         daily_usage = df_merged.groupby("date")["LogID"].count().reset_index(name="count_usage")
#         chart_daily_usage = (
#             alt.Chart(daily_usage)
#             .mark_line(point=True)
#             .encode(
#                 x=alt.X("date:T", title="Date"),
#                 y=alt.Y("count_usage:Q", title="Daily Logs"),
#                 tooltip=["date","count_usage"]
#             )
#             .properties(height=300, width=700, title="Daily Logs Over Time")
#         )
#         st.altair_chart(chart_daily_usage, use_container_width=True)

#         daily_anom_df = df_merged[df_merged["predicted_anomaly"]==1]
#         if not daily_anom_df.empty:
#             daily_anom = daily_anom_df.groupby("date")["LogID"].count().reset_index(name="count_anomalies")
#             chart_daily_anom = (
#                 alt.Chart(daily_anom)
#                 .mark_line(point=True)
#                 .encode(
#                     x=alt.X("date:T", title="Date"),
#                     y=alt.Y("count_anomalies:Q", title="Daily Anomalies"),
#                     tooltip=["date","count_anomalies"]
#                 )
#                 .properties(height=300, width=700, title="Daily Anomalies Over Time")
#             )
#             st.altair_chart(chart_daily_anom, use_container_width=True)
#         else:
#             st.write("No daily anomalies found.")
#     else:
#         st.write("No valid Timestamp column after unification, or no non-null Timestamps found.")
...
#         (df_merged["predicted_anomaly"] == 1) &
#         (df_merged.get("is_unusual_hour", False) == True)
#     ]
