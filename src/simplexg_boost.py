...
def setup_logging(log_path='logs/simple_gradient_boosting.log'):
...
def balance_data_SMOTE(X_train, y_train, random_state=42):
    ...
    try:
        ...
        plt.savefig('models/simple_gradient_boosting_smote_distribution.png')
        plt.close()
        logging.info("SMOTE distribution plot saved at 'models/simple_gradient_boosting_smote_distribution.png'.")
        print("SMOTE distribution plot saved at 'models/simple_gradient_boosting_smote_distribution.png'.")
        return X_res, y_res
    except Exception as e:
...
def train_gradient_boosting(X_train, y_train, random_state=42):
    ...
    try:
        ...
        print(f"Error: Training Gradient Boosting model failed with error: {e}")
        raise

def evaluate_model(model, X_test, y_test, model_name='Gradient Boosting'):
    ...
    try:
        ...
        plt.ylabel('Actual')
        plt.savefig(f'models/simple_gradient_boosting_confusion_matrix.png')
        plt.close()
        logging.info(f"{model_name} confusion matrix plot saved at 'models/simple_gradient_boosting_confusion_matrix.png'.")
        print(f"{model_name} confusion matrix plot saved at 'models/simple_gradient_boosting_confusion_matrix.png'.")
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'models/simple_gradient_boosting_precision_recall_curve.png')
        plt.close()
        logging.info(f"{model_name} Precision-Recall curve plot saved at 'models/simple_gradient_boosting_precision_recall_curve.png'.")
        print(f"{model_name} Precision-Recall curve plot saved at 'models/simple_gradient_boosting_precision_recall_curve.png'.")
...
def adjust_threshold(model, X_test, y_test, desired_recall=0.90, model_name='Gradient Boosting'):
    """
    ...
    try:
        ...
        predictions_df.to_csv('data/simple_gradient_boosting_predictions_adjusted_threshold.csv', index=False)
        print(f"{model_name} Adjusted threshold predictions saved successfully at 'data/simple_gradient_boosting_predictions_adjusted_threshold.csv'.")
        logging.info(f"{model_name} Adjusted threshold predictions saved successfully at 'data/simple_gradient_boosting_predictions_adjusted_threshold.csv'.")
...
def plot_feature_importance(model, X, model_name='Gradient Boosting'):
    """
    ...
    try:
        ...
        plt.ylabel('Features')
        plt.tight_layout()
        plt.savefig(f'models/simple_gradient_boosting_feature_importances.png')
        plt.close()
        
        logging.info(f"{model_name} feature importance plot saved at 'models/simple_gradient_boosting_feature_importances.png'.")
        print(f"{model_name} feature importance plot saved at 'models/simple_gradient_boosting_feature_importances.png'.")
    except Exception as e:
...
def save_model(model, model_path='models/simple_gradient_boosting_model.joblib'):
...
def save_initial_predictions(model, X_test, y_test, predictions_save_path='data/simple_gradient_boosting_predictions.csv'):
...
def main():
    ...
    model_save_path = "models/simple_gradient_boosting_model.joblib"
    predictions_save_path = "data/simple_gradient_boosting_predictions.csv"
    ...
    best_gb_model = hyperparameter_tuning(X_train_res, y_train_res, model_name='Gradient Boosting')

    # Train the model with the best parameters
    gb_model = best_gb_model.fit(X_train_res, y_train_res)
    logging.info("Gradient Boosting model trained with best parameters.")
    ...
    # Adjust threshold
    adjust_threshold(gb_model, X_test, y_test, desired_recall=0.90, model_name='Gradient Boosting')

    # Plot feature importances
    plot_feature_importance(gb_model, X_train_res, model_name='Gradient Boosting')

    # Save the trained model
    save_model(gb_model, model_path=model_save_path)

    # Save initial predictions
    save_initial_predictions(gb_model, X_test, y_test, predictions_save_path=predictions_save_path)
